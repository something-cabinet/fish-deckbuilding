...
- cross-blitz
status: approved
implementation_notes: 'Tech-stack note: the Architecture Impact Matrix and NFR-1 below describe the original Excalibur.js/Svelte implementation and are stale � the project is now on Next.js/React/TypeScript. Design intent (overworld, campaign, card types, mana, save/load) in this spec remains valid; only the old tech references (Excalibur.js, Svelte) are outdated.'
---
